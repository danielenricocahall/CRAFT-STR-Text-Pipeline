...
if __name__ == '__main__':
    ...
    parser.add_argument('--trained_craft_model', default='./CRAFT_models/craft_mlt_25k.pth', type=str, help='trained_craft_model')
    parser.add_argument('--trained_str_model', default='./STR_models/TPS-ResNet-BiLSTM-Attn-case-sensitive.pth', type=str, help='trained_str_model')
    ...
    for k, image_path in enumerate(image_list):
        ...
        bboxes, polys, score_text = predict(net, img, opt.text_threshold, opt.link_threshold, opt.low_text, opt.cuda, opt.poly, opt.canvas_size, opt.mag_ratio)
        images = []
        for bbox in bboxes:
            x, y = bbox[0]
            x, y = int(x), int(y)
